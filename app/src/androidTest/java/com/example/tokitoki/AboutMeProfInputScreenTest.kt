...
@HiltAndroidTest
class AboutMeProfInputScreenTest {
    @get:Rule(order = 0)
    ...
    @Test
    fun aboutMeProfInputScreenIsDisplayed() = runBlocking {
        ...
        composeTestRule.onNodeWithText("内容に同意して次へすすむ").performClick()

        composeTestRule.onNodeWithTag(TestTags.ABOUT_ME_PROF_INPUT_CONTENTS).assertIsDisplayed()

        return@runBlocking
    }

    @Test
    fun aboutMeProfInputScreenSwipeTest() = runBlocking {
        composeTestRule.setContent {
            TokitokiTheme {
                TokitokiNavGraph(startDestination = TokitokiDestinations.ABOUT_ME_PROF_INPUT_ROUTE)
            }
        }
        composeTestRule.onNodeWithTag(TestTags.ABOUT_ME_PROF_INPUT_MY_SELF_PAGER).isDisplayed()

        composeTestRule.onNodeWithText("タイプ１").performClick()

        composeTestRule.onNodeWithTag(TestTags.ABOUT_ME_PROF_INPUT_MY_SELF_PAGER).performTouchInput {
            swipeLeft()
        }

        composeTestRule.awaitIdle()

        composeTestRule.onNodeWithText("タイプ2").performClick()
        ...
        composeTestRule.onNodeWithText("タイプ3").performClick()
